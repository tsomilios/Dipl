...
float DFRobot_EC::readEC(float voltage, float temperature)// voltage = averagevoltage
{
    float value = 0,valueTemp = 0,valuet= 0;
    ...
    value = voltage / (1.0+0.0185*(temperature-25.0));  //temperature compensation
    //Serial.println(value );
    if(value<50)Serial.println("No solution!");   //25^C 1413us/cm<-->about 216mv  if the voltage(compensate)<150,that is <1ms/cm,out of the range
    else if(value>3300)Serial.println("Out of the range!");  //>20ms/cm,out of the range
    else
    {
      if(value<=448)valuet=6.84*value-64.32;   //1ms/cm<EC<=3ms/cm
      else if(value<=1457)valuet=6.98*value-127;  //3ms/cm<EC<=10ms/cm
      else valuet=5.3*value+2278;                           //10ms/cm<EC<20ms/cm
      valuet/=1000;    //convert us/cm to ms/cm
      //Serial.print(valuet,2);  //two decimal
      //Serial.println("ms/cm");
    } 
    this->_ecvalue = valuet;                           //store the EC value for Serial CMD calibration
    return valuet;
}
